...
class mkf(Command):
    ...
    Creates a file with the name <filename> and opens it with $EDITOR.
    """
    ...
    def execute(self):
        ...
        if not lexists(filename):
            self.fm.execute_console(f"touch {filename}")
        else:
            self.fm.notify("file exists!", bad=True)
...
class fzf_rga_documents_search(Command):
    ...
    def execute(self):
        if self.arg(1):
            ...
            msg = "Usage: fzf_rga_search_documents <search string>"
            self.fm.notify(msg, bad=True)
            return

        import subprocess
        import os.path
        from ranger.container.file import File
        cmd1 = f"rga --rga-adapters=pandoc,poppler '{search_string}' . "
        cmd2 = "fzf +m --preview 'echo {}'"
        cmd3 = "awk -F':' '{print $1}'"
...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        if fd is not None:
            ...
            exclude = "--no-ignore-vcs "
            exclude += "--exclude '.git' "
            exclude += "--exclude '*.py[co]' "
